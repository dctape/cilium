...
// *Warning*: Calling this function requires to caller to properly setup
...
func (m *Map) DumpWithCallback(cb DumpCallback) error {
	...
	for {
		err := GetNextKeyWithUBA(m.fd, bpfCurrentKeyPtr, bpfCurrentKeySize)

		if err != nil {
			break
		}

		err = LookupElementWithUBA(m.fd, bpfNextKeyPtr, bpfNextKeySize)
...
func (m *Map) DumpReliablyWithCallback(cb DumpCallback, stats *DumpStats) error {
	...
	for stats.Lookup = 1; stats.Lookup <= stats.MaxEntries; stats.Lookup++ {
		...
		nextKeyValid := GetNextKeyWithUBA(m.fd, bpfNextKeyPtr, bpfNextKeySize)
		err := LookupElementWithUBA(m.fd, bpfCurrentKeyPtr, bpfCurrentKeySize)
		if err != nil {
			stats.LookupFailed++
